...
const MainSection = () => {
  ...
  const [skillItemCount, setSkillItemCount] = useState([
    { id: 0, value: 80 },
    { id: 1, value: 80 },
    { id: 2, value: 60 },
  ])
    ...
    <section>
      ...
      <div className="work" >
        ...
        <div className="about-me-header">
          <svg id="work-svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            <path
      ...
      <div className="work education" >
        ...
        <div className="about-me-header">
          <svg id="education-svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            <path
      ...
      <div className="skills">
        <svg id="skills-svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
          <path
        ...
        <div className="skills-items">
          {
            skillItemCount.map(item => (
              <div key={item.id} className="skills-item">
                <button
                  className="delete-item-button"
                  value={item.id}
                  onClick={e => {
                    let newState = skillItemCount.filter(item => item.id !== +e.target.value)
                    setSkillItemCount(newState)
                  }}
                >x
            </button>
                <Text initialValue="skill-set" textType="skill" />
                <input
                  className="skill-input"
                  type="text"
                  name={item.id}
                  placeholder="Enter value eg. 55"
                  onChange={(e) => {
                    const state = skillItemCount.map(skill => {
                      if (skill.id === +e.target.name) {
                        skill.value = e.target.value
                      }
                      return skill
                    })
                    setSkillItemCount(state)
                  }}
                />
                <div className="progress-bar">
                  <div
                    className="progress" style={{ width: `${item.value}%` }}></div>
                </div>
              </div>
            ))
          }
        ...
        <button
          ...
          onClick={() => {
            ...
            setSkillItemCount([...skillItemCount, { id: newId, value: 50 }])
            console.log(skillItemCount)
          }}
        >add item
        </button>
