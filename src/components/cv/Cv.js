...
const Cv = () => {
  ...
  //add link icons 
  const addLinkIcons = (element, iconPath, correctionTop) => {
    var iconData = new Image()
    iconData.src = iconPath
  ...
  const savePdf = () => {
    ...
    //hide link.text to be able to add real working links
    ...
    doc.addHTML(document.getElementById('pdf'), async function () {
      ...
      let correctionForLinks = 0;
      document.getElementById("link-items").childNodes.forEach(link => {
        addLinkIcons(`${link.id}`, `img/${link.id}.png`, correctionForLinks)
        correctionForLinks += 0.7;
      })
      doc.save("CV.pdf");
      await (setDisplayLinks(true))
  ...
  return (
    <div id="pdf">
      ...
      <div style={{ display: 'flex' }}>
        <SideInfo setLinks={setLinks} links={links} displayLinks={displayLinks} />
        <MainSection />
      </div>
      <button onClick={savePdf} className="download-button">download</button>
